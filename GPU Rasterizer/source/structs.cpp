...
inputVertex::inputVertex(const FPoint3& pos, const FVector3& normal, const FVector2& uv)
	:position{pos}
	,normal{normal}
	,uv{uv}
{
...
TextureManager::TextureManager(const std::string& diffusePath, const std::string& normalMapPath, const std::string& specularMapPath, const std::string& glossyMapPath, ID3D11Device* pDevice, float shininess)
	:diffuseTexture{ diffusePath.c_str(),pDevice }
	, normalMap{ normalMapPath.c_str(), pDevice }
	, specularMap{ specularMapPath.c_str(), pDevice }
	, glossMap{ glossyMapPath.c_str(), pDevice }
	, shininess{ shininess }{
}
