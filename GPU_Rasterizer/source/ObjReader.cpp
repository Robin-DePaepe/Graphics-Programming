		...
		while (!reader.eof())
		{
			...
			if (IsLineValid(line))
			{
				...
				if (line[0] == 'f')
				{
					...
					{
						inputVertex vertex{ m_Vertices.at(numbers[i][0] - 1),m_VertexNormals.at(numbers[i][2] - 1),m_VertexTextures.at(numbers[i][1] - 1) };

						std::pair<bool, int> dublicate{ mesh.CheckForDublicate(vertex) };

						if (!dublicate.first) //test if its a  dublicates
						{
							indices.push_back(int((mesh.AddVertex(vertex)))); //not a dublicate
						}
						else
						{
					...
					mesh.AddIndices(IPoint3{ indices.at(0),indices.at(1),indices.at(2) });
				}
...
void ObjReader::ParseVertexNormal(const std::string& data)
{
	...
	//the normals are not always normalized when reading in a file
	Normalize(numbers);
